...
head(experiment.aggregate@raw.data)
```
...
```{r}
JackStrawPlot(object = experiment.aggregate, PCs = 15:20)
```
...
colnames(means2) <- c("mean.in.cluster", "mean.out.of.cluster")
markers_all2 <- cbind(head(markers_all), means)
markers_all2_pos <- subset(markers_all2, avg_logFC > 0)
```
